        ...
        if (req.getCocoonRequest().getMethod().equalsIgnoreCase("GET")) {
            ...
            if (req.getCocoonRequest().getParameter("actionbuttondelete") != null) {
                ...
                if (!validationmessages.isEmpty()) {
                    ...
                    // If users has applied a new URI to the topic. Insert where old and then remove old. Make sure the URI is valid and not-existing first.
                    if (req.getCocoonRequest().getParameter("old-the-resource") != null && !req.getCocoonRequest().getParameter("old-the-resource").equalsIgnoreCase(req.getCocoonRequest().getParameter("the-resource"))) {
                        String sparql1 = "INSERT INTO <" + SettingsService.getProperty("sublima.basegraph") + "> {\n" +
                                "<" + req.getCocoonRequest().getParameter("the-resource") + "> ?p ?o . }\nWHERE {\n" +
                                "<" + req.getCocoonRequest().getParameter("old-the-resource") + "> ?p ?o . }";

                        String sparql2 = "INSERT INTO <" + SettingsService.getProperty("sublima.basegraph") + "> {\n" +
                                "?p ?o <" + req.getCocoonRequest().getParameter("the-resource") + "> . }\nWHERE {\n" +
                                "?p ?o <" + req.getCocoonRequest().getParameter("old-the-resource") + "> . }";

                        String sparql3 = "DELETE FROM <" + SettingsService.getProperty("sublima.basegraph") + "> {\n" +
                                "?p ?o <" + req.getCocoonRequest().getParameter("old-the-resource") + "> . }\nWHERE {\n" +
                                "?p ?o <" + req.getCocoonRequest().getParameter("old-the-resource") + "> . }";

                        String sparql4 = "DELETE FROM <" + SettingsService.getProperty("sublima.basegraph") + "> {\n" +
                                "<" + req.getCocoonRequest().getParameter("old-the-resource") + "> ?p ?o  . }\nWHERE {\n" +
                                "<" + req.getCocoonRequest().getParameter("old-the-resource") + "> ?p ?o  . }";

                        sparulDispatcher.query(sparql1);
                        sparulDispatcher.query(sparql2);
                        sparulDispatcher.query(sparql3);
                        sparulDispatcher.query(sparql4);

                        logger.trace("Updated topic URI for existing posts.");
                    ...
                    parameterMap.remove("old-the-resource");
                    if (parameterMap.get("subjecturi-prefix") != null) {
                        parameterMap.put("subjecturi-prefix", new String[]{getProperty("sublima.base.url") +
                                parameterMap.get("subjecturi-prefix")[0]});
                    }
