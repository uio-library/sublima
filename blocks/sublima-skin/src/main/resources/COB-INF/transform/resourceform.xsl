    ...
    <form action="{$baseurl}/admin/ressurser/ny" method="POST">
      ...
      <table>
        ...
        <tr>
          ...
          <td>
            <select id="country" name="dct:coverage" multiple="multiple" size="10">
              <xsl:for-each select="/c:page/c:content/c:allcountries/rdf:RDF/go:Country">
                <xsl:sort lang="{$interface-language}" select="./rdfs:label[@xml:lang=$interface-language]"/>
                <xsl:choose>
                  <xsl:when test="./@rdf:about = /c:page/c:content/c:resourcedetails/c:resource/rdf:RDF/sub:Resource/dct:coverage/@rdf:resource">
                    <xsl:if test="./rdfs:label[@xml:lang=$interface-language]">
                      <option value="{./@rdf:about}" selected="selected">
                        <xsl:value-of select="./rdfs:label[@xml:lang=$interface-language]"/>
                      </option>
